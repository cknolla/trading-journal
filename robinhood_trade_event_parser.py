...
trade_events = []
with open(os.path.join('raw_data', 'trade_events.txt')) as input_file:
    default_trade_event = {
        'options': []
    }
    ...
    for line in input_file:
        ...
        if expiration_line := re.search(r'(\w+) \$(\d+\.?\d*) (\w+) Expiration', line):
            ...
            for expiration_detail_line in input_file:
                ...
                if contracts_line := re.match(r'Contracts$', expiration_detail_line):
                    quantity_line = re.match(r'(\d+)$', input_file.readline())
                    option_data['quantity'] = int(quantity_line.group(1))
                elif date_line := re.match(r'Date$', expiration_detail_line):
            ...
            for exercise_detail_line in input_file:
                if contracts_line := re.match(r'Contracts$', exercise_detail_line):
                    quantity_line = re.match(r'(\d+)$', input_file.readline())
                    option_data['quantity'] = int(quantity_line.group(1))
                elif price_at_expiration_line := re.search(r'\w+ Price at Expiration', exercise_detail_line):
                ...
                elif date_line := re.match(r'Date$', exercise_detail_line):
                    exercise_date_line = re.match(r'(\d+)\/(\d+)\/(\d+)$', input_file.readline())
                    month = exercise_date_line.group(1).zfill(2)
                    day = exercise_date_line.group(2).zfill(2)
                    year = exercise_date_line.group(3)
            ...
            for assignment_detail_line in input_file:
                if contracts_line := re.match(r'Contracts$', assignment_detail_line):
                    quantity_line = re.match(r'(\d+)$', input_file.readline())
                    option_data['quantity'] = int(quantity_line.group(1))
                elif price_at_expiration_line := re.search(r'\w+ Price at Expiration', assignment_detail_line):
                    price_line = re.match(r'\$(\d+\.?\d*)$', input_file.readline())
                    price = round(float(price_line.group(1)), 2)
                    option_data['price'] = round(abs(price - option_data['strike']), 2)
                elif date_line := re.match(r'Date$', assignment_detail_line):
                    exercise_date_line = re.match(r'(\d+)\/(\d+)\/(\d+)$', input_file.readline())
                    month = exercise_date_line.group(1).zfill(2)
                    day = exercise_date_line.group(2).zfill(2)
                    year = exercise_date_line.group(3)
...
with open(os.path.join('trade_events_data', 'trade_events.json'), 'w') as output_file:
